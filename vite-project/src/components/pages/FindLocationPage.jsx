...
const FindLocationPage = () => {
  ...
  // Handle redirection to the weather details page
  const handleViewDetails = () => {
    navigate('/fire-vulner', { state: { coordinates } }); // Pass weatherData as state
  };

  useEffect(() => {
    // Scroll instantly to the far right when the page loads
    window.scrollTo({
      left: document.documentElement.scrollWidth,  // Scroll to the right end of the page
      top: 0,  // Keep the vertical position at the top
    });
  }, []);

  return (
    <div className="bg-gray-700 min-h-screen w-screen flex justify-center items-center">
